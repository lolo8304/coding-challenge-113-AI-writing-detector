...
public class InboundLoggingInterceptor implements HandlerInterceptor {
    ...
    private static final String ATTR_LOG_REQUEST_BODY = InboundLoggingInterceptor.class.getName() + ".logRequestBody";
    ...
    public boolean preHandle(
        ...
        if (effective.logRequest() && !monitoring) {
            if (request.getContentLength() > 0) {
                // Body is read by MVC later; log it in afterCompletion once cached.
                request.setAttribute(ATTR_LOG_REQUEST_BODY, Boolean.TRUE);
            } else {
    ...
    public void afterCompletion(
        ...
        if (effective.logRequest() && !monitoring && Boolean.TRUE.equals(request.getAttribute(ATTR_LOG_REQUEST_BODY))) {
